...
from PythonTesting.src.inventory import (
    Inventory,
...
def ten_stock_inventory():
    ...
    inventory.add_new_stock("Rebook Test", 25.50, 2)
    return inventory
...
def test_buy_and_sell_nikes_adidas():
    ...
    # Remove 1 sweatpants to sell to the nex customer
...
@pytest.mark.parametrize("name,quantity,exception,new_quantity,new_total", [
    ("Puma Test", 0, InvalidQuantityException("Cannot remove a quantity of 0. Must remove at least 1 item"), 0, 0),
    ("Not Here", 5, ItemNotFoundException("Could not find Not Here in our stocks. Cannot remove non-existing stock"), 0, 0),
    ("Puma Test", 25, InvalidQuantityException("Cannot remove these 25 items. Only 8 items are in stock"), 0, 0),
...
def test_remove_stock(ten_stock_inventory, name, quantity, exception, new_quantity, new_total):
    ...
    except (InvalidQuantityException, ItemNotFoundException) as inst:
        assert isinstance(inst, type(exception))
        assert inst.args == exception.args
    else:
